	...
	async.waterfall([
		...
		function (commitObj, cb) {
			...
			if (_.isObject(commitObj)) {
				redisClient.hset(fileKey, commitObj.r, cb);
			} else {
		...
		initialCommit = commitObj || initialCommit;
		docRevs[chanel] = [initialCommit];
		callback && callback(null, initialCommit);
	});
}
...
function getFileRevs(chanel, cb) {
	cb && cb(null, docRevs[chanel] || []);
}
...
function getRevById(chanel, revId, cb) {
	...
	if (!r) {
		redisClient.hget(getFileKey(chanel), revId, function (err, commitObj) {
			cb && cb(err, commitObj ? JSON.parse(commitObj) : {});
		});
	} else {
		cb && cb(null, r);
	}
